...
export async function GET(req: NextRequest) {
  try {
    const { dbClient } = await getAuthenticatedClient(req);
    ...
    const { data, error } = await dbClient
      .from("messages")
      .select(
        `id, sender_id, content, tts, created_at, sender:profiles!messages_sender_id_fkey(username, display_name, avatar_url, tier)`
      )
      .eq("thread_type", "global")
